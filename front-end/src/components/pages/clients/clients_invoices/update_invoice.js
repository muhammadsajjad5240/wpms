// below we import modules and declare some global variables for the component
...
class UpdateClientInvoice extends Component {
  ...
  componentDidMount = () => {
    ...
    // below we send request to the server when the comonent load to get invoice from the database
    axios.post("/Invoices/getInvoice", {
      id: pramerter.id
    })
      .then(res => {
        // response from server is data which we assign to the state below
        this.setState({ project: res.data })
        for (let i in this.state.project) {
          obj = this.state.project[i];
        };
        this.setState({ proj_name: obj[0].projectName })
        this.setState({ slip_id: obj[0].slip_id })
        this.setState({ client_name: obj[0].clientName })
        this.setState({ proj_desc: obj[0].project_description })
        this.setState({ proj_strt_date: obj[0].project_start_date })
        projectID = obj[0].projectId
        this.setState({ proj_end_date: obj[0].project_end_date })
        this.setState({ proj_amount: obj[0].project_amount })
        this.setState({ paid: obj[0].paid })
        this.setState({ paymentType: obj[0].paymentType })
        type = obj[0].paymentType;
        this.setState({ type: obj[0].project_type })
        this.setState({ technology: obj[0].project_technoloy })
      })
      // below we console the error in case of error occur
      .catch(error => { console.log(error) });
    if (type === "Full") {
      // document.getElementById('amount_type').readOnly = true;
      alert('Hi');
    }
  }
  // below we create method which will call when the form will submut
  handleSubmit = (e) => {
    // below we prevent the form from reloading
    e.preventDefault();
    ...
    if (!namePattern.test(projectName) || projectName === "") {
      ...
      // below we send request to the server to update the invoice
      axios.post('/Invoices/updateInvoice', {
      ...
      axios.post('/Projects/updateProjectAmount', {
        ...
        .then(res => {
          ...
          if (a === "OK") {
            // axios.post('/Salary/newSlipHistory', slipHistory)
            let salary_words = converter.toWords(Number(this.state.proj_amount)); // => “thirteen”
            ...
            doc.text(120, 60, 'PAYSLIP FOR PROJECT ' + this.state.proj_name.substr(0, 20));
            // End of main heading 
            ...
            doc.setTextColor('#000')
            doc.setFontType("bold");
            doc.text(20, 680, 'Total Amount')
            doc.setFontType("normal");
            doc.setFontSize(10)
            doc.setTextColor('#000')
            doc.text(110, 680, String(this.state.proj_amount) + ' ( ' + String(salary_words) + ' )')

            // start of employeer signature
            doc.setFontSize(10)
            doc.setTextColor('#000')
            doc.setFontType("normal")
            doc.text(100, 780, 'Employer Signature')
            // End of employeer signature
            // start of employeer signature
            doc.setFontSize(10)
            doc.setTextColor('#000')
            doc.setFontType("normal");
            doc.text(360, 780, 'CEO Signature')
            // End of employeer signature
            // Save the Data
      ...
      //   technology: ''
      // })
      // below we replace our location
    }
  }
  // below we create method that will call when the use focus in the input field
  myfocus = (a) => {
        ...
        <form>
          ...
          <div className='form-row ml-1'>
            <div class="form-group row col-md-6">
              ...
              <div class="col-md-7 col-sm-12 col-xs-12">
                <input type='text' placeholder='Enter Project Name' id='proj_name' readOnly onFocus={() => this.myfocus('proj_name')} value={this.state.proj_name} onChange={this.handleProjectName} />
              </div>
            </div>
            <div class="form-group row col-md-6">
              <label for="clientName" class="col-md-5 col-form-label">Client Name</label>
              <div class="col-md-7 col-sm-12 col-xs-12">
                <input type='text' placeholder='Enter Client Name' id='client_name' readOnly onFocus={() => this.myfocus('client_name')} value={this.state.client_name} onChange={this.handleClientName} />
              </div>
            </div>
          </div>
          <div className='form-row ml-1'>
            <div class="form-group row col-md-12">
              ...
              <div class="col-md-9 col-sm-12 col-xs-12">
                <textarea id='proj_description' placeholder='Project Description' readOnly onFocus={() => this.myfocus('proj_description')} style={{ width: "102%", marginLeft: "-6%" }} value={this.state.proj_desc}></textarea>
              </div>
            </div>
          </div>
          <div className='form-row ml-1'>
            <div class="form-group row col-md-6">
              ...
              <div class="col-md-7 col-sm-12 col-xs-12">
                <input type="date" class="form-control" id="strt_date" name='StrtDate' readOnly onFocus={() => this.myfocus('strt_date')} value={this.state.proj_strt_date} />
              </div>
            </div>
            <div class="form-group row col-md-6">
              <label for="endDate" class="col-md-5 col-form-label">Project End Date</label>
              <div class="col-md-7 col-sm-12 col-xs-12">
                <input type="date" class="form-control" id="end_date" name='endDate' readOnly onFocus={() => this.myfocus('end_date')} value={this.state.proj_end_date} />
              </div>
            </div>
          </div>
          <div className='form-row ml-1'>
            <div class="form-group row col-md-6">
              ...
              <div class="col-md-7 col-xs-12 col-sm-12">
                <input type='text' placeholder='Enter Project Type' id='proj_type' readOnly onFocus={() => this.myfocus('proj_type')} value={this.state.type} />
              </div>
            </div>
            <div class="form-group row col-md-6">
              <label for="technology" class="col-md-5 col-form-label">Project Technology</label>
              <div class="col-md-7 col-xs-12 col-sm-12">
                <input type='text' placeholder='Enter Project Technology' id='proj_technology' readOnly onFocus={() => this.myfocus('proj_technology')} value={this.state.technology} />
              </div>
            </div>
          </div>
          <div className='form-row ml-1'>
            <div class="form-group row col-md-6">
              ...
              <div class="col-md-7 col-xs-12 col-sm-12">
                <input type='text' placeholder='Enter Project Amount' id='proj_amount' readOnly onFocus={() => this.myfocus('proj_amount')} value={this.state.proj_amount} />
              </div>
            </div>
            <div class="form-group row col-md-6">
          ...
          <div className='row'>
            <div className='col-sm-2 offset-sm-5'>
              <input type="submit" class="btn btn-primary" className='submitbtn' onClick={this.handleSubmit} value='Update' id='_print' name='print' title='Update' />
            </div>
          </div>
        </form>
      </div>
...
export default UpdateClientInvoice;